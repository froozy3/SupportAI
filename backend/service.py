...
async def retreive_relevant_faq(question: str, faq_embeddings, top_k=2) -> list[str]:
...
async def get_answer(session: AsyncSession, ask: UserAsk) -> UserResponse:
    ...
    faq_embeddings = await get_embeddings(FAQ_DATA)
    relevant_faq = await retreive_relevant_faq(question, faq_embeddings)
    answer = await request_to_AI(question, relevant_faq)
    history = History(
        question=question,
